...
if ($className.ToLower().EndsWith("entity")) {
	$className = $className.Substring($className.Length-6);
}
...
$outputPath = "Entities\" + "$className" + "Entity"
...
$foundModelType = Get-ProjectType ($className + "Entity") -Project $Project
if (!$foundModelType) { 
